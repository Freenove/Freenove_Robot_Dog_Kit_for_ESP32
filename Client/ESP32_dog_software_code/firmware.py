...
from ui_firmware import Ui_widget_firmware #导入串口界面对象
from Command import COMMAND                #导入esptool指令参数

class ESP32_Firmware(QtWidgets.QWidget, Ui_widget_firmware):
    pyserial_chn = QtCore.pyqtSignal(int)
    def __init__(self):
        super(ESP32_Firmware, self).__init__()
        self.setupUi(self)
        #添加界面的logo
    ...
    def upload_bin(self):
        if self.combobox_firmware_port.currentText() != '':
            print("\nStart uploading robot dog firmware to ESP32.")
